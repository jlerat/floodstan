...
transformed parameters {
  ...
  // Reduced censor
  real ucensor = genlogistic_cdf(ycensor | ylocn, yscale, yshape1);
  real vcensor = genlogistic_cdf(zcensor | zlocn, zscale, zshape1);
  ...
  for(i in 1:Ncases[1, 1]) {
    uv[i11[i], 1] = genlogistic_cdf(y[i11[i]] | ylocn, yscale, yshape1);
    uv[i11[i], 2] = genlogistic_cdf(z[i11[i]] | zlocn, zscale, zshape1);
  }

  // .. cases with y observed only
  for(i in 1:Ncases[1, 2])
    uv[i12[i], 1] = genlogistic_cdf(y[i12[i]] | ylocn, yscale, yshape1);
  
  for(i in 1:Ncases[1, 3])
    uv[i13[i], 1] = genlogistic_cdf(y[i13[i]] | ylocn, yscale, yshape1);

  // .. cases with z observed only
  for(i in 1:Ncases[2, 1])
    uv[i21[i], 2] = genlogistic_cdf(z[i21[i]] | zlocn, zscale, zshape1);

  for(i in 1:Ncases[3, 1])
    uv[i31[i], 2] = genlogistic_cdf(z[i31[i]] | zlocn, zscale, zshape1);
}
...
model {
  ...
  if(Ncases[1, 1] > 0) {
    target += copula_lpdf(uv[i11,:] | rho);
    print("log density #1.a =", target());
    target += genlogistic_lpdf(y[i11] | ylocn, yscale, yshape1);
    print("log density #1.b =", target());
    target += genlogistic_lpdf(z[i11] | zlocn, zscale, zshape1);
    print("log density #1.c =", target());
  }
  
  print("log density #2 =", target());

  // Case 21 : y censored and z observed
  if(Ncases[2, 1] > 0) {
    target += copula_lpdf_conditional(uv[i21, 2], ucensor, rho);
    target += genlogistic_lpdf(z[i21] | zlocn, zscale, zshape1);
  }

  // Case 31 : y is missing and z is observed. 
  // this is marginal pdf for z
  if(Ncases[3, 1] > 0) 
    target += genlogistic_lpdf(z[i31] | zlocn, zscale, zshape1);

  // Case 12 : y observed and z censored
  // we re-use the expression of case 12 and invert u and v variables
  if(Ncases[1, 2] > 0) {
    target += copula_lpdf_conditional(uv[i12, 1], vcensor, rho);
    target += genlogistic_lpdf(y[i12] | ylocn, yscale, yshape1);
  }

  // Case 22 : both y and z censored. Copulas cdf times 
  // the number of occurences for 22
  if(Ncases[2, 2] > 0)
     target += Ncases[2, 2]*copula_lcdf(uvcensors | rho);

  // Case 32 : y is missing and z is censored 
  // this a marginal cdf for z reduced variable times
  // the number of occurences for 32
  if(Ncases[3, 2] > 0)
     target += Ncases[3, 2] * genlogistic_lcdf(zcensor | zlocn, zscale, zshape1);
  
  // Case 13 : y is obs, z is missing
  // this is marginal pdf for y (analogous to case 31)
  if(Ncases[1, 3] > 0)
     target += genlogistic_lpdf(y[i13] | ylocn, yscale, yshape1);

  // Case 23 : y is cens, z is missing
  // this is marginal cdf for y (analogous to case 32)
  if(Ncases[2, 3] > 0)
     target += Ncases[2, 3] * genlogistic_lcdf(ycensor | ylocn, yscale, yshape1);
  
  print("log density #3 =", target());
}
