...
def cdf2(h1, h2, r):
    ...
    for i in range(5):
        rr = r*x[i]
        rr2 = 1.-rr*rr
        bv += w[i]*math.exp((rr*h3 - h12)/rr2)/math.sqrt(rr2)

    bv = bv*r + norm.cdf(h1)*norm.cdf(h2)
...
plt.close("all")
fig, axs = plt.subplots(ncols=2, figsize=(15, 6), layout="tight")
