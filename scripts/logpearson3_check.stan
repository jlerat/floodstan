...
generated quantities {
  ...
  for(i in i11){
    tmp[1] = y[i];
    luncens[i] = logpearson3_lpdf(tmp | ylocn, yscale, yshape1);
    cens[i] = logpearson3_cdf(y[i] | ylocn, yscale, yshape1);
    lcens[i] = log(cens[i]);
  }

  // Log-likelihood for univariate censored model
  // Case 11 : y observed
  real loglikelihood = logpearson3_lpdf(y[i11] | ylocn, yscale, yshape1);

  // Case 21 : y censored 
  if(Ncases[2, 1] > 0)
     loglikelihood += Ncases[2, 1] * log(logpearson3_cdf(ycensor | ylocn, yscale, yshape1));
